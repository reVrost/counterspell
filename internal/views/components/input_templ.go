...
func InputBar(projects map[string]views.UIProject) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 18, "</div></div></div><!-- Divider --><div class=\"w-px h-4 bg-gray-700 mx-1\"></div><!-- Submit/Voice Button --><button type=\"button\" @click=\"if(text.length > 0) { $refs.voiceForm.requestSubmit(); }\" @mousedown=\"if (text.length === 0 && !app.isRecording) app.startVoiceRecording($refs.input)\" @mouseup=\"if (app.isRecording) app.stopVoiceRecording()\" @mouseleave=\"if (app.isRecording) app.stopVoiceRecording()\" @touchstart.prevent=\"if (text.length === 0 && !app.isRecording) app.startVoiceRecording($refs.input)\" @touchend.prevent=\"if (app.isRecording) { app.stopVoiceRecording(); } else if (text.length > 0) { $refs.voiceForm.requestSubmit(); }\" @touchcancel=\"if (app.isRecording) app.cancelRecording()\" :class=\"app.isRecording ? 'bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.4)] scale-110' : (text.length > 0 ? 'bg-blue-500 text-white shadow-[0_0_15px_rgba(59,130,246,0.4)]' : 'bg-gray-700 text-gray-300 hover:bg-gray-600')\" class=\"w-8 h-8 rounded-lg flex items-center justify-center text-sm transition-all duration-200 ml-1 select-none\"><i class=\"fas\" :class=\"text.length > 0 ? 'fa-arrow-up' : (app.isRecording ? 'fa-microphone animate-pulse' : 'fa-microphone')\"></i></button></div></div></form></div>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return nil
