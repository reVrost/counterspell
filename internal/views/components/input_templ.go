...
func InputBar(projects map[string]views.UIProject) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		for _, p := range projects {
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/views/components/input.templ`, Line: 165, Col: 83}
			}
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/views/components/input.templ`, Line: 167, Col: 90}
			}
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/views/components/input.templ`, Line: 172, Col: 98}
			}
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/views/components/input.templ`, Line: 173, Col: 65}
			}
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/views/components/input.templ`, Line: 175, Col: 113}
			}
		...
		for _, m := range uiModels {
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/views/components/input.templ`, Line: 210, Col: 58}
			}
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/views/components/input.templ`, Line: 212, Col: 52}
			}
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/views/components/input.templ`, Line: 213, Col: 67}
			}
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 18, "</div></div></div><!-- Divider --><div class=\"w-px h-4 bg-gray-700 mx-1\"></div><!-- Submit/Voice Button --><button type=\"button\" @click=\"if(text.length > 0) { navigator.vibrate && navigator.vibrate(30); $refs.voiceForm.requestSubmit(); text = ''; $refs.input.style.height = 'auto'; }\" @mousedown=\"if (text.length === 0 && app && !app.isRecording) app.startVoiceRecording($refs.input)\" @mouseup=\"if (app && app.isRecording) app.stopVoiceRecording()\" @mouseleave=\"if (app && app.isRecording) app.stopVoiceRecording()\" @touchstart.prevent=\"if (text.length === 0 && app && !app.isRecording) { navigator.vibrate && navigator.vibrate(50); app.startVoiceRecording($refs.input); }\" @touchend.prevent=\"if (app && app.isRecording) { navigator.vibrate && navigator.vibrate([30, 50, 30]); app.stopVoiceRecording(); } else if (text.length > 0) { navigator.vibrate && navigator.vibrate(30); $refs.voiceForm.requestSubmit(); text = ''; $refs.input.style.height = 'auto'; }\" @touchcancel=\"if (app && app.isRecording) app.cancelRecording()\" :class=\"(app && app.isRecording) ? 'bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.4)] scale-110' : (text.length > 0 ? 'bg-blue-500 text-white shadow-[0_0_15px_rgba(59,130,246,0.4)]' : 'bg-gray-700 text-gray-300 hover:bg-gray-600')\" class=\"w-8 h-8 rounded-lg flex items-center justify-center text-sm transition-all duration-200 ml-1 select-none\"><i class=\"fas\" :class=\"text.length > 0 ? 'fa-arrow-up' : ((app && app.isRecording) ? 'fa-microphone animate-pulse' : 'fa-microphone')\"></i></button></div></div></form></div>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return nil
