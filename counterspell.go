...
import (
	...
	api "github.com/revrost/counterspell/pkg/api"
...
func AddToEcho(e *echov4.Echo, opts ...Option) error {
	...
	slog.Info("Counterspell installed successfully with Echo v4", slog.String("db_path", cfg.dbPath))
	return nil
}
...
func AddToEchoV5(e *echov5.Echo, opts ...Option) error {
	...
	slog.Info("Counterspell installed successfully with Echo v5", slog.String("db_path", cfg.dbPath))
	return nil
}
...
func AddToStdlib(mux *http.ServeMux, opts ...Option) (*Counterspell, error) {
	...
	slog.Info("Counterspell installed successfully", slog.String("db_path", cfg.dbPath))
	return cs, nil
}
...
func setupObservability(db *sql.DB, serviceName, serviceVersion string) (*Counterspell, error) {
	exporter := NewSQLiteSpanExporter(db)
	logWriter := NewSQLiteLogWriter(db)
...
func setupGlobalLogger(logWriter *SQLiteLogWriter) {
	stdoutHandler := slog.NewTextHandler(os.Stdout, &slog.HandlerOptions{})
	jsonDBHandler := slog.NewJSONHandler(logWriter, &slog.HandlerOptions{})
	h := slogmulti.Fanout(stdoutHandler, jsonDBHandler)
...
type tracingAttrs struct{ ctx context.Context }

func (t tracingAttrs) with(ctx context.Context, attrs []slog.Attr) []slog.Attr {
...
func createStdlibLogsHandler(apiHandler *api.APIHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		handleStdlibEchoV4Adapter(w, r, apiHandler.QueryLogs)
	}
}

func createStdlibTracesHandler(apiHandler *api.APIHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		handleStdlibEchoV4Adapter(w, r, apiHandler.QueryTraces)
	}
}

func createStdlibTraceDetailsHandler(apiHandler *api.APIHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
...
func shutdownCounterspell(cs *Counterspell) {
	slog.Info("Counterspell shutting down...")
	...
	if err := cs.logWriter.Close(); err != nil {
		slog.Error("Failed to close log writer", slog.Any("error", err))
	}
	...
	slog.Info("Counterspell shutdown complete")
}
