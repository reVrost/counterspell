...
{#if taskStore.todos.length > 0}
  ...
  <div class="fixed bottom-24 right-4 z-30">
    <button
      ...
      class="group flex items-center gap-2 h-9 pl-3 pr-4 bg-popover hover:bg-card border border-white/[0.08] hover:border-violet-500/30 rounded-full shadow-xl shadow-black/40 transition-all"
    >
      <!-- Progress circle -->
  ...
  >
    ...
    >
      ...
      >
        <div class="flex items-center gap-2">
          <div class="w-6 h-6 rounded-lg bg-violet-500/10 flex items-center justify-center">
            <ListChecksIcon class="w-3 h-3 text-violet-400" />
          </div>
      ...
      <div class="h-0.5 bg-gray-800">
        <div
          class="h-full bg-gradient-to-r from-violet-500 to-green-500 transition-all duration-300"
        ...
        {#each taskStore.todos as todo, index}
          <div
            class={cn(
              ...
              todo.status === 'in_progress' && 'bg-violet-500/10 border border-violet-500/20',
          ...
          >
            ...
            <div class="w-5 h-5 shrink-0 flex items-center justify-center mt-0.5">
              ...
              {:else if todo.status === 'in_progress'}
                <div
                  class="w-4 h-4 rounded-full bg-violet-500/20 flex items-center justify-center relative"
                >
                  <div class="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse"></div>
                  <div
                    class="absolute inset-0 rounded-full border border-violet-400/50 animate-ping"
