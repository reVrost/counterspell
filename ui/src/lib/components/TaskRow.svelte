	...
	{#if variant === 'completed'}
		...
		<div class="flex justify-between items-center w-full">
			<div class="flex items-center gap-3">
				...
				<div class="min-w-0">
					<div class="text-sm text-gray-400 leading-snug line-clamp-2">{task.title}</div>
					<div class="text-xs text-gray-600 mt-0.5">{project?.name ?? 'Unknown'}</div>
				</div>
			</div>
	...
	{:else}
		...
		<div class="flex justify-between items-start mb-2">
			<div class="flex items-center gap-2">
				...
				<span class="text-sm font-medium text-gray-400">{project?.name ?? 'Unknown'}</span>
			</div>
		...
		<p class="text-sm text-gray-200 font-medium leading-snug line-clamp-2" class:pr-6={variant === 'review'}>
			{task.title}
		</p>
